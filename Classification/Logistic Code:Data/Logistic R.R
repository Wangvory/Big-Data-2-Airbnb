#### Table 10.2
setwd('/Users/zhoujiawang/Desktop/Brandeis Life/BigData2/Logistic Code:Data')
...
barplot(aggregate(delays.df$Flight.Status == "delayed", by = list(delays.df$DEST), 
                  mean, rm.na = T)[,2], xlab = "DEST", ylab = "Average Delay", 
        names.arg = c("EWR", "JFK", "LGA"))

barplot(aggregate(delays.df$Flight.Status == "delayed", by = list(delays.df$DEP_TIME %/% 100), 
                  mean, rm.na = T)[,2], xlab = "departure time", ylab = "Average Delay", 
        names.arg = c("1":"21"))
# strange: not supposed to be earlier than 6:00am
...
# Try fewer, essential predictors
...
# sensitivity perfect, but specificity????
...
summary(reg)
# also show odds ratio
data.frame(odds = exp(coef(reg))) 

#### Table 10.10 

summary(logit.reg)
# also show odds ratio
data.frame(odds = exp(coef(logit.reg)))
